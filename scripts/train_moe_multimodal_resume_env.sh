...
# 恢复训练脚本（使用环境变量方法）：从checkpoint继续训练
#
# 这个方法通过环境变量传递checkpoint路径，避免Hydra解析等号的问题
#
# 使用方法：
# bash scripts/train_moe_multimodal_resume_env.sh <checkpoint_path> [max_epochs]

# 检查参数
if [ -z "$1" ]; then
    echo "错误: 必须提供checkpoint路径"
    echo "用法: bash scripts/train_moe_multimodal_resume_env.sh <checkpoint_path> [max_epochs]"
    exit 1
fi
...
# 使用环境变量传递checkpoint路径，避免Hydra解析问题
export PL_RESUME_CKPT_PATH="$CHECKPOINT_PATH"
...
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python $NAVSIM_DEVKIT_ROOT/navsim/planning/script/run_training.py \
  ...
  +trainer.params.ckpt_path="${PL_RESUME_CKPT_PATH}" \
